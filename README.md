...
## 1. Project Overview
This repository contains a comprehensive numerical simulation and validation study of turbulent flow in a circular pipe at a Reynolds number of Re = 10,000. The primary objective is to evaluate the predictive accuracy of various turbulence models (specifically SST k-ω and Realizable k-ε) and examine the impact of near-wall grid resolution (y+ study) against analytical formulations and the empirical Haaland correlation (1983).
...
* **Aspect Ratio (L/D):** 80
* **Entrance Length (Theoretical):** Fully developed flow occurs after L/D ≈ 13.59 (L ≈ 2.72 m).

### Fluid Properties & Flow Conditions
* **Fluid Density (ρ):** 1,000 kg/m³
* **Dynamic Viscosity (μ):** 0.02 Pa·s (or $\text{kg}/(\text{m}\cdot\text{s})$)
* **Mean Inlet Velocity (V_avg):** 1.0 m/s
* **Reynolds Number (Re):**
...
Numerical results for the Darcy friction factor (f), skin friction coefficient (Cf), and pressure drop (ΔP) are validated using the following classical formulations:
...
4. **Pressure Drop (ΔP) - Darcy-Weisbach Equation:**
   ...
   In the fully developed region (from z = 6 m to z = 16 m, L_fd = 10 m):
...
* **y+ ≈ 1 Mesh:** Requires a first-cell height of y = 0.000318 m (0.318 mm) to fully resolve the viscous sublayer.
* **y+ ≈ 10 Mesh:** First-cell height set to y = 0.00318 m (3.18 mm), utilizing wall functions.
...
| Parameter | CFD (SST, y+ ≈ 1) | CFD (RKE, y+ ≈ 1) | Empirical (Haaland) | SST Error (%) | RKE Error (%) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Darcy Friction Factor (f)** | $0.0325$ | $0.0341$ | $0.030886$ | **5.22%** | **10.40%** |
| **Skin Friction Coefficient (Cf)** | $0.00812$ | $0.00853$ | $0.00772$ | **5.18%** | **10.49%** |
| **Pressure Drop (ΔP_6-16m)** | $811.70\text{ Pa}$ | $855.16\text{ Pa}$ | $772.15\text{ Pa}$ | **5.12%** | **10.75%** |
...
### Key Observations
* **Turbulence Model Performance:** The Shear Stress Transport (SST) $k$-$\omega$ model displays superior accuracy (within ~5% error) compared to the Realizable k-ε (RKE) model (~11% error) in capturing near-wall gradients.
* **Grid Resolution:** Resolving the viscous sublayer (y+ ≈ 1) is critical for accurate near-wall velocity profiles. While $y^+ \approx 10$ with wall functions predicts bulk pressure drop effectively, it oversimplifies near-wall velocity distribution.
...
## 7. Conclusions
1. The **SST k-ω model** is the recommended choice for wall-bounded turbulent pipe flows at moderate Reynolds numbers, yielding predictions within 5% of Haaland's correlation.
...
3. Proper viscous sublayer grid spacing (y+ ≈ 1) is mandatory for predicting wall-bounded friction parameters accurately.
